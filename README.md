...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-Built%20with%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Informly%2FGo)

The Informly Go library provides convenient access to the Informly APIs from Go.
...
```go
response, err := client.Contacts.CreateContact(...)
if err != nil {
...
```go
response, err := client.Contacts.WithRawResponse.CreateContact(...)
if err != nil {
    return err
}
...
response, err := client.Contacts.CreateContact(ctx, ...)
```
...
response, err := client.Contacts.CreateContact(ctx, request, ...)
```
